...
export async function playRoutedAudio(file: Blob, cue: any, sinkId: string, onEnded: () => void) {
  const context = new AudioContext();
  ...
  const solos = new Set(cue.soloInputs || []), muted = new Set(cue.mutedInputs || []);
  routes.forEach((outputs, input) => outputs.forEach((output: number) => { const crosspoint = context.createGain(), enabled = !muted.has(input + 1) && (!solos.size || solos.has(input + 1)), gang = cue.audioGangs?.[input], leader = gang ? cue.audioGangs.findIndex((value: number) => value === gang) : input, level = (cue.audioLevels?.[`${input + 1}-${output}`] ?? 0) + (cue.inputTrims?.[String(leader + 1)] || 0); crosspoint.gain.value = enabled ? 10 ** (level / 20) : 0; splitter.connect(crosspoint, input); crosspoint.connect(merger, 0, Math.max(0, output - 1)); }));
  let tail: AudioNode = merger;
  const effects = cue.effects?.length ? cue.effects : [{ type: "highpass", value: cue.highpass, enabled: cue.highpass > 20 }, { type: "lowpass", value: cue.lowpass, enabled: cue.lowpass < 20000 }, { type: "compressor", enabled: cue.compressor }, { type: "delay", value: cue.delay, feedback: cue.feedback, enabled: cue.delay > 0 }];
  for (const effect of effects.filter((item: any) => item.enabled !== false)) {
    let node: AudioNode | null = null;
    if (["highpass", "lowpass", "peaking", "lowshelf", "highshelf"].includes(effect.type)) { const filter = context.createBiquadFilter(); filter.type = effect.type; filter.frequency.value = effect.frequency || effect.value || 1000; filter.Q.value = effect.q || 1; filter.gain.value = effect.gain || 0; node = filter; }
    if (effect.type === "compressor") { const compressor = context.createDynamicsCompressor(); compressor.threshold.value = effect.threshold ?? -24; compressor.ratio.value = effect.ratio ?? 12; node = compressor; }
    if (effect.type === "gain") { const effectGain = context.createGain(); effectGain.gain.value = 10 ** ((effect.value || 0) / 20); node = effectGain; }
    if (effect.type === "delay") { const delay = context.createDelay(5), feedback = context.createGain(); delay.delayTime.value = Math.min(5, effect.value || 0); feedback.gain.value = Math.min(.95, Math.max(0, effect.feedback || 0)); delay.connect(feedback).connect(delay); node = delay; }
    if (effect.type === "distortion") { const wave = context.createWaveShaper(), amount = Math.max(0, effect.value || 0); wave.curve = Float32Array.from({ length: 44100 }, (_, index) => { const x = index * 2 / 44100 - 1; return (3 + amount) * x * 20 * Math.PI / (Math.PI + amount * Math.abs(x)); }); wave.oversample = "4x"; node = wave; }
    if (node) { tail.connect(node); tail = node; }
  }
  const mainTrim = context.createGain(); mainTrim.gain.value = 10 ** ((cue.mainTrim || 0) / 20); tail.connect(mainTrim).connect(gain);
  const trimSplit = context.createChannelSplitter(outputChannels), finalMerger = context.createChannelMerger(outputChannels); gain.connect(trimSplit); for (let output = 0; output < outputChannels; output++) { const trim = context.createGain(); trim.gain.value = 10 ** ((cue.outputTrims?.[String(output + 1)] || 0) / 20); trimSplit.connect(trim, output); trim.connect(finalMerger, 0, output); } finalMerger.connect(context.destination);
  const loops = Math.max(0, Number(cue.loops) || 0);
  const markers = (cue.slices || []).filter((slice) => slice.at > trimStart && slice.at < trimEnd).sort((a, b) => a.at - b.at), points = [trimStart, ...markers.map((slice) => slice.at), trimEnd];
  const segments = points.slice(0, -1).map((start, index) => ({ start, end: points[index + 1], count: index ? Number(markers[index - 1].count ?? 1) : 1 })).filter((slice) => slice.count !== 0);
  const onePlay = segments.some((slice) => slice.count < 0) ? 0 : segments.reduce((sum, slice) => sum + (slice.end - slice.start) * slice.count / rate, 0), duration = loops && onePlay ? onePlay * loops : 0;
  if (cue.preservePitch) {
    const url = URL.createObjectURL(file), media = document.createElement("audio"), sourceNode = context.createMediaElementSource(media); let play = 0, segment = 0, segmentPlay = 0;
    media.src = url; media.playbackRate = rate; (media as any).preservesPitch = true; sourceNode.connect(splitter);
    const start = async (offset = 0) => { const slice = segments[segment]; if (!slice) { play++; segment = 0; segmentPlay = 0; if (!loops || play < loops) return start(); onEnded(); return; } media.currentTime = slice.start + offset; await media.play(); };
    media.ontimeupdate = () => { const slice = segments[segment]; if (!slice || media.currentTime < slice.end - .015) return; media.pause(); segmentPlay++; if (slice.count < 0 || segmentPlay < slice.count) start(); else { segment++; segmentPlay = 0; start(); } };
    if (cue.fadeIn) gain.gain.setValueAtTime(0, context.currentTime), gain.gain.linearRampToValueAtTime(Math.max(0, Number(cue.volume) || 0) / 100, context.currentTime + Math.min(cue.fadeIn, duration || cue.fadeIn));
    if (cue.fadeOut && duration) gain.gain.setValueAtTime(gain.gain.value, context.currentTime + Math.max(0, duration - cue.fadeOut)), gain.gain.linearRampToValueAtTime(0, context.currentTime + duration);
    if (segments.length) await start(); else queueMicrotask(onEnded);
    return { audio: context, gain, media, duration, pause: () => media.pause(), resume: () => media.play(), seek: (elapsed: number) => { let position = Math.max(0, elapsed) % (onePlay || 1); segment = 0; for (; segment < segments.length; segment++) { const each = (segments[segment].end - segments[segment].start) / rate; if (position < each) { media.currentTime = segments[segment].start + position * rate; break; } position -= each; } }, stop: async () => { media.ontimeupdate = null; media.pause(); URL.revokeObjectURL(url); if (context.state !== "closed") await context.close(); } };
  }
  let source: AudioBufferSourceNode, stopped = false, play = 0, segment = 0, segmentPlay = 0, generation = 0;
  const start = (offset = 0) => {
    const slice = segments[segment];
    if (!slice) { play++; segment = 0; segmentPlay = 0; if (!loops || play < loops) return start(); onEnded(); return; }
    const currentGeneration = generation; source = context.createBufferSource(); source.buffer = buffer; source.playbackRate.value = rate; source.connect(splitter); source.onended = () => { if (stopped || generation !== currentGeneration) return; segmentPlay++; if (slice.count < 0 || segmentPlay < slice.count) start(); else { segment++; segmentPlay = 0; start(); } }; source.start(0, slice.start + offset, Math.max(.001, slice.end - slice.start - offset));
  };
  if (cue.fadeIn) gain.gain.setValueAtTime(0, context.currentTime), gain.gain.linearRampToValueAtTime(Math.max(0, Number(cue.volume) || 0) / 100, context.currentTime + Math.min(cue.fadeIn, duration || cue.fadeIn));
  if (cue.fadeOut && duration) gain.gain.setValueAtTime(gain.gain.value, context.currentTime + Math.max(0, duration - cue.fadeOut)), gain.gain.linearRampToValueAtTime(0, context.currentTime + duration);
  if (segments.length) start(); else queueMicrotask(onEnded);
  return {
    audio: context, gain, duration,
    pause: () => context.suspend(), resume: () => context.resume(),
    seek: (elapsed: number) => { if (!onePlay || !segments.length) return; generation++; if (source) { source.onended = null; try { source.stop(); } catch {} } let position = Math.max(0, elapsed) % onePlay; play = Math.floor(Math.max(0, elapsed) / onePlay); segment = 0; segmentPlay = 0; for (; segment < segments.length; segment++) { const slice = segments[segment], each = (slice.end - slice.start) / rate, total = slice.count < 0 ? Infinity : each * slice.count; if (position < total) { segmentPlay = Math.floor(position / each); return start((position % each) * rate); } position -= total; } start(); },
...
export async function startLtc(fps = 30, startSeconds = 0, level = 0.8, sinkId = "") {
  ...
  const node = new AudioWorkletNode(audio, "stagecue-ltc", { outputChannelCount: [1], processorOptions: { fps, drop: fps === 29.97, start: startSeconds } }), gain = audio.createGain();
...
export function scheduleMidiFile(buffer: ArrayBuffer, output: { send: (message: number[], timestamp?: number) => void }, rate = 1) {
  const data = new Uint8Array(buffer);
  if (String.fromCharCode(...data.slice(0, 4)) !== "MThd") throw new Error("This is not a Standard MIDI File.");
  const division = (data[12] << 8) | data[13], smpte = Boolean(division & 0x8000), smpteFps = smpte ? 256 - data[12] : 0, ticksPerFrame = smpte ? data[13] : 0;
  ...
  const milliseconds = (tick: number) => { if (smpte) return tick / (smpteFps * ticksPerFrame) * 1000; let lastTick = 0, tempo = 500000, microseconds = 0; for (const change of tempos) { if (change.tick > tick) break; microseconds += (change.tick - lastTick) * tempo / division; lastTick = change.tick; tempo = change.tempo; } return (microseconds + (tick - lastTick) * tempo / division) / 1000; };
  const started = performance.now(); let maxTime = 0;
  for (const event of events) { const at = milliseconds(event.tick) / Math.max(.01, rate); output.send(event.message, started + at); maxTime = Math.max(maxTime, at); }
  return maxTime / 1000;
}

export function startVideoFx(canvas: HTMLCanvasElement, video: HTMLVideoElement, effects: any[] = []) {
  const gl = canvas.getContext("webgl2", { premultipliedAlpha: false });
  if (!gl) return null;
  const shader = (type: number, source: string) => { const value = gl.createShader(type)!; gl.shaderSource(value, source); gl.compileShader(value); if (!gl.getShaderParameter(value, gl.COMPILE_STATUS)) throw new Error(gl.getShaderInfoLog(value) || "Video shader failed."); return value; };
  ...
  const amount = (type: string, fallback: number) => Number(effects.find((item) => item.enabled !== false && item.type === type)?.value ?? fallback), uniform = gl.getUniformLocation(program, "fx"); let frame = 0;
  const draw = () => { if (video.readyState >= 2) { const width = video.videoWidth || 1, height = video.videoHeight || 1; if (canvas.width !== width || canvas.height !== height) { canvas.width = width; canvas.height = height; gl.viewport(0, 0, width, height); } gl.bindTexture(gl.TEXTURE_2D, texture); gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, video); gl.uniform4f(uniform, amount("brightness", 100) / 100, amount("contrast", 100) / 100, amount("saturate", 100) / 100, amount("hue-rotate", 0)); gl.drawArrays(gl.TRIANGLES, 0, 6); } frame = requestAnimationFrame(draw); };
  draw(); return () => { cancelAnimationFrame(frame); gl.deleteTexture(texture); gl.deleteBuffer(buffer); gl.deleteProgram(program); };
}

export const collaborationSocket = (url: string, room: string, onMessage: (value: any) => void) => {
